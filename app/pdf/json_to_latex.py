...
atbd_journal_tex: Final[str] = f"{here}/ATBD_JOURNAL.tex"
...
def json_to_latex(atbd_doc: Dict, tmp_dir: str, journal: bool) -> Tuple[str, str]:
    """
    ...
    if journal:
        copyfile(atbd_journal_tex, f"{tmp_dir}/ATBD_JOURNAL.tex")
    else:
    ...
    with NamedTemporaryFile(dir=tmp_dir, mode="w") as json_fp:
        ...
        json_fp.write(json.dumps(atbd_doc, default=str))
        json_fp.flush()
        completed: Union[CompletedProcess[bytes], CompletedProcess[Any]] = run(
            args=[serialize_py, tmp_name, str(journal)],
            capture_output=True,
            cwd=tmp_dir,
            encoding=encoding,
        )
