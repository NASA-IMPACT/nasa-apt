...
CREATE FUNCTION apt.copy_atbd(original_id integer, OUT new_id integer, OUT created_atbd apt.atbds)
AS $$
BEGIN
...
FROM  (
   ...
   FROM   apt.atbds t WHERE atbd_id = original_id
...
FROM  (
   SELECT t #= hstore('atbd_id', new_id::text) AS t1
   FROM   apt.atbd_versions t WHERE atbd_id = original_id
   ) sub;
...
FROM  (
   SELECT t #= hstore('atbd_id', new_id::text) AS t1
   FROM   apt.citations t WHERE atbd_id = original_id
   ) sub;
...
FROM  (
   SELECT t #= hstore('atbd_id', new_id::text) AS t1
   FROM   apt.data_access_input_data t WHERE atbd_id = original_id
   ) sub;
...
FROM  (
   SELECT t #= hstore('atbd_id', new_id::text) AS t1
   FROM   apt.data_access_output_data t WHERE atbd_id = original_id
   ) sub;
...
FROM  (
   SELECT t #= hstore('atbd_id', new_id::text) AS t1
   FROM   apt.data_access_related_urls t WHERE atbd_id = original_id
   ) sub;
...
FROM  (
   SELECT t #= hstore('atbd_id', new_id::text) AS t1
   FROM   apt.publication_references t WHERE atbd_id = original_id
   ) sub;
...
FROM  (
   SELECT t #= hstore('atbd_id', new_id::text) AS t1
   FROM   apt.algorithm_implementations t WHERE atbd_id = original_id
   ) sub;
...
FROM  (
   ...
   FROM   apt.algorithm_input_variables t WHERE atbd_id = original_id
   ) sub;
...
FROM  (
   ...
   FROM   apt.algorithm_output_variables t WHERE atbd_id = original_id
   ) sub;
