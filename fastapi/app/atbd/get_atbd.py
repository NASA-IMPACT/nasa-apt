...
rest_api_endpoint: str = environ.get('REST_API_ENDPOINT') or sys.exit('REST_API_ENDPOINT env var required')
...
def get_atbd(atbd_id: Optional[int] = None, alias: Optional[str] = None) -> Dict:
    ...
    if alias:
        ...
        from_column = 'atbd_id'
        q = atbd_id
    url: str = f'{rest_api_endpoint}/atbds?{from_column}=eq.{q}&select=*,contacts(*),contact_groups(*),atbd_versions(atbd_id,atbd_version,status)&limit=1' if alias else f'{rest_api_endpoint}/atbds?atbd_id=eq.{atbd_id}&select=*,contacts(*),contact_groups(*),atbd_versions(atbd_id,atbd_version,status)&limit=1'
